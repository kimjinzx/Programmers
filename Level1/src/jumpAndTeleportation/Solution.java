...
public class Solution {
        public int solution(int n) {
            ...
            while (n != 0) { // 아래의 조건문 및 연산식을 통해 n이 나올때 까지 반복실행
                if (n % 2 == 0) {
                    n = n / 2;
                }else {
                    n--;
            ...
            // 문제 구조상 최소한의 점프의 회수만 구하면 됨
            // 점프를 최소화 하기 위해선 순간이동을 최대한으로 사용해야함
            // 점프를 사용해야만 하는 경우 = n이 홀수일 때
            // 위의 식에선 연산식을 역순으로 푸는것과 같음
            // 홀수일 경우 answer를 +1하고, n을 -1하여 짝수로 만들어주어 다시(반복) 연산
            // 짝수일 경우 n을 나누기 2하여 다시(반복) 연산
            return answer;
    }
}
